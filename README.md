# Hola, soy Lautaro Vicente Guibelalde | Software Engineer

Me apasiona construir soluciones digitales robustas y escalables. Soy **Ingeniero en Sistemas de Información**, lo que me permite abordar el desarrollo de software con una visión analítica, enfocada en la arquitectura y la eficiencia del sistema.

### 🛠️ Mi Stack Tecnológico

#### **Backend Development** 
- **Python**: Experiencia en desarrollo de APIs, automatización y scripting.
- **JavaScript/Node.js**: Construcción de servicios rápidos y escalables.
- **Bases de Datos**: Diseño de modelos de datos eficientes (SQL/NoSQL).

#### **Frontend Development** 
- **React & Vite**: Creación de interfaces modernas, dinámicas y de alto rendimiento.
- **Tailwind CSS**: Diseño UI/UX minimalista y responsivo basado en utilidad.

#### **Engineering Tools & Methodologies**
- **Version Control**: Manejo avanzado de **Git** (Gitflow, Rebase, Pull Requests).
- **Containerization**: Experiencia con **Docker** para entornos consistentes.
- **Project Management**: Metodologías ágiles (**Scrum/Kanban**).
- **Environment**: Configuración de entornos de despliegue en **Vercel**, **Netlify** y **AWS**.
- **Documentation**: Diagramado de arquitectura de sistemas y bases de datos.

### Tecnologías en detalle
![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white)
![Linux](https://img.shields.io/badge/-Linux-FCC624?style=flat-square&logo=linux&logoColor=black)

### Contacto & Colaboraciones
Como Ingeniero, valoro la comunicación clara y el trabajo en equipo. ¡Hablemos!

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/lautaro-vicente-guibelalde/)
[![WhatsApp](https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)](https://wa.me/549295468471)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](lautarovicente@hotmail.com)

---
*"El software es una combinación de arte e ingeniería."*
